...
DEFAULT_CONFIG = {
    "site_title": "MAHEN FF PREMIUM HOSTING WEB",
    "site_header": "MAHEN PREMIUM VPS",
    ...
    "passwords": {
        "secret": hashlib.sha256("MHNSU89".encode()).hexdigest(),
...
BOT_TOKEN = "__BOT_TOKEN_PLACEHOLDER__"
...
async def api_check(message: types.Message):
    ...
    try:
        ...
        r = requests.get(url, timeout=15, headers={
            'User-Agent': 'Mozilla/5.0 (MAHEN VPS Bot)'
        })
...
if __name__ == "__main__":
    asyncio.run(main())
'''.replace("__BOT_TOKEN_PLACEHOLDER__", token)
        ...
        # Pin a stable Python version so Railway installs prebuilt wheels
        # for aiohttp/pydantic-core instead of compiling them from source
        # (which needs a Rust/gcc toolchain that isn't available and
        # causes 'linker cc not found' / 'maturin failed' build errors).
        with open(os.path.join(server_path, "runtime.txt"), 'w') as f:
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("MAHEN PREMIUM VPS - Starting...")
    print("=" * 50)
    print(f"Port: {port}")
    print(f"Debug: {debug}")
    ...
    print("  Secret: MHNSU89 (Can change user password)")
